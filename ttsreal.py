...
class LocalEdgeTTS(BaseTTS):
    ...
    def _init_local_tts(self):
        """初始化本地TTS"""
        try:
            from local_edge_tts import LocalEdgeTTS as LocalTTS
            self.local_tts = LocalTTS(self.voice_name)
            logger.info("本地EdgeTTS初始化成功")
        except ImportError as e:
            logger.error(f"无法导入本地EdgeTTS模块: {e}")
            logger.error("请确保已安装edge-tts库: pip install edge-tts")
            self.local_tts = None
        except Exception as e:
            logger.error(f"本地EdgeTTS初始化失败: {e}")
            self.local_tts = None
    
    def set_voice(self, voice_name: str):
        """设置语音"""
        if self.local_tts:
            self.local_tts.set_voice(voice_name)
            self.voice_name = voice_name
    
    def get_available_voices(self):
        """获取可用语音列表"""
        if self.local_tts:
            return self.local_tts.get_available_voices()
        return {}
    
    def txt_to_audio(self, msg):
        """文本转语音"""
        if not self.local_tts:
            logger.error("本地EdgeTTS未初始化，无法进行语音合成")
            return
        ...
        try:
            # 强制使用备选TTS，避免网络问题
            if hasattr(self.local_tts, 'fallback_tts') and self.local_tts.fallback_tts:
                logger.info("强制使用备选TTS引擎...")
                audio_data = self.local_tts._fallback_text_to_speech_sync(text)
            else:
                # 如果备选TTS不可用，尝试使用原来的方法
                audio_data = self.local_tts.text_to_speech_sync(text)
            
            logger.info(f'-------local edge tts time:{time.time()-t:.4f}s')
            
            if not audio_data:
                logger.error('本地EdgeTTS转换失败，无音频数据')
                return
            ...
            logger.error(f'本地EdgeTTS转换失败: {e}')
            logger.exception('local_edgetts')
    
    def __create_bytes_stream(self, byte_stream):
        """创建字节流"""
        stream, sample_rate = sf.read(byte_stream)  # [T*sample_rate,] float64
        logger.info(f'[INFO]local tts audio stream {sample_rate}: {stream.shape}')
        stream = stream.astype(np.float32)
...
class VoitsTTS(BaseTTS):
    ...
    def gpt_sovits(self, text, reffile, reftext,language, server_url) -> Iterator[bytes]:
        ...
        #req["stream_chunk_size"] = stream_chunk_size  # you can reduce it to get faster response, but degrade quality
        req["stream"] = True
        req["refer_wav_path"] = reffile
        req["prompt_text"] = reftext
        # req["refer_wav_path"] = 'zzx1.wav'
        # req["prompt_text"] = '儿时叫卖声，作者：张未。'
        req["prompt_language"] = language
