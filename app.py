...
def llm_response(message,nerfreal):
    # 检查是否是醒来命令
    if "春儿醒来" in message:
        print("检测到醒来命令，播放醒来动画后进入对话")
        # 先切换到醒来状态
        nerfreal.set_curr_state(3, True)
        # 等待醒来动画播放完成
        time.sleep(2)  # 根据实际动画长度调整等待时间
        # 切换到说话状态并回应
        nerfreal.set_curr_state(0, True)
        nerfreal.put_msg_txt("爸爸，我在呢，有什么事")
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tts', type=str, default='local_edgetts') #local_edgetts edgetts xtts gpt-sovits cosyvoice fishtts
